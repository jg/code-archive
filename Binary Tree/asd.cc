...
#define DEBUG 1
// #define PR(p) print(p)
#define FOREACH(m) for (IT it = m.begin(); it != m.end(); ++it) 
#define PR(p) cout << p->first << ", " << p->second << endl;
...
TreeMap::TreeMap( const TreeMap& m ) {/*{{{*/
  if ( DEBUG ) cout << this << "." << "TreeMap(" << &m << ")" << endl;
  root = NULL;
  iterator it1 = m.begin();
  // unsafe_insert(PAIR(1,"10"));
  for ( ; it1 != m.end(); ++it1 ) {
    // cout << it1->first << endl;
    insert(*it1);
  }
...
std::pair<TreeMap::iterator, bool> TreeMap::insert(const std::pair<Key, Val>& entry) {/*{{{*/
  if ( DEBUG ) cout << this << "." << "insert(" << entry.first << ", " << entry.second << ")" << endl;
  // cout << endl;
  // PR(entry);
  // cout << endl;
...
TreeMap::iterator TreeMap::unsafe_insert(const std::pair<Key, Val>& entry) {/*{{{*/
  // PR(entry);
  if ( DEBUG ) cout << this << "." << "unsafe_insert(" << entry.first << ", " << entry.second << ")" << endl;
// Inserts an element into the map.
...
TreeMap::iterator TreeMap::find(const Key& k) {/*{{{*/
  if ( DEBUG ) cout << this << "." << "find(" << k << ")" << endl;
...
TreeMap::Val& TreeMap::operator[](const Key& k) {/*{{{*/
  if ( DEBUG ) cout << this << ".operator[" << k << "]" << endl;
...
bool TreeMap::struct_eq(const TreeMap& another) const/*{{{*/ {
  if ( DEBUG ) cout << this << "." << "struct_eq(" << &another << ")" << endl;
  // cout << "STUCT_EQ"; 
  iterator it1 = begin();
  iterator it2 = another.begin();

  for ( ; it1 != end(); it1++ ) {
    it2 = another.find(it1->first);
    if ( it2 == another.end() ) return false;
    if ((it1.node->left == NULL) &&  (it2.node->left != NULL)     || 
        (it1.node->left != NULL) &&  (it2.node->left == NULL)     ||
...
bool TreeMap::info_eq(const TreeMap& another) const/*{{{*/ {
  if ( DEBUG ) cout << this << "." << "info_eq(" << &another << ")" <<  endl;
  // cout << "INFO_EQ"; 
  iterator it1 = begin();
  iterator it2 = another.begin();

  for ( ; it1 != end(); it1++ ) 
    if ( another.find(it1->first) == another.end() ) return false;
...
TreeMap::const_iterator TreeMap::begin() const { /*{{{*/
  // if ( DEBUG ) cout << "const_begin()" << endl;
  if ( root == NULL ) return iterator(NULL);
...
// 0x27f68.find(30)
// 0x27f68.unsafe_insert(30, )
// operator[]
// 0x27f68.find(15)
// 0x27f68.unsafe_insert(15, )
// copying constructor
// 0x27f48.insert(15, 15)
// 0x27f48.unsafe_insert(15, 15)
// 0x27f48.insert(30, 30)
// 0x27f48.unsafe_insert(30, 30)
// 0x27f68.info_eq(0x27f68)
// 0x27f68.struct_eq(0x27f68)
// 0x27f68.info_eq(0x27f48)
// 0x27f68.struct_eq(0x27f48)

  m.unsafe_insert(PAIR(30, ""));
  m.unsafe_insert(PAIR(15, ""));

  // TEST_MAP k(m);
  TEST_MAP k(m);

  k.insert(PAIR(15, "15"));
  k.unsafe_insert(PAIR(15, "15"));
  k.insert(PAIR(30, "30"));
  k.unsafe_insert(PAIR(30, "30"));

  // assert( m.info_eq(k) == true );
  // assert( k.info_eq(m) == true );
